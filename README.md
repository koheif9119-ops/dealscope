...
- **成長可能性資料の収集**：分類キーワードに「成長可能性に関する事項」を追加（グロース企業の事業計画・成長可能性資料が中計タブに入る）。
- **過去検索タブ**：蓄積した全期間の開示を、キーワード・ジャンル・期間で横断検索できる。データは `data/monthly/`（月別まとめ）から読み込む。※TDnetの原文PDFリンクは公開から約1か月で切れる（一覧・タイトルは無期限に残る）。
- **さかのぼり取得（backfill）**：過去365日分のTDnet開示を一括取得するワークフロー。原則1回だけ実行済み。再実行は Actions タブ →「backfill」→「Run workflow」（取得済みの日は自動で飛ばすので何度実行しても安全）。
- **朝のダイジェスト**：平日8:45にIssue「📬 朝のダイジェスト」へ新着のM&A・中計一覧をコメント投稿し、GitHubの通知メールが届く。通知が来ない場合はリポジトリ右上の「Watch」を「All Activity」にする。
